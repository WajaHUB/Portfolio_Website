...
class Board {
    ...
    /**Create Another function, go back to MoveNode.
     * @return chipList
     * @param aPiece
     *  MoveNode will call on these functions to create MoveNodes */
    public ArrayList<Integer> chipList(PieceColor aPiece) {
    ...
    public ArrayList<String> legalList(ArrayList<Integer> theChipList) {
        ArrayList<String> legalMoveList = new ArrayList<>();
        int sourceRow, sourceColumn;
        char sourceRowChar, sourceColumnChar;
        char rowChar, colChar;
        String moveString;
        char baseRowChar, baseColChar;
        baseRowChar = (char) ('1' - 2);
        baseColChar = (char) ('a' - 2);
        for (int cellIndex: theChipList) {
            sourceRow = cellIndex / EXTENDED_SIDE;
            sourceColumn = cellIndex % EXTENDED_SIDE;
            sourceRowChar = (char) (sourceRow + baseRowChar);
            sourceColumnChar = (char) (sourceColumn + baseColChar);
            for (int destrow = sourceRow - 2;
                 destrow <= sourceRow + 2;
                 destrow++) {
                for (int destcolumn = sourceColumn - 2;
                    destcolumn <= sourceColumn + 2; destcolumn++) {
                    rowChar = (char) (baseRowChar + destrow);
                    colChar = (char) (baseColChar + destcolumn);
                    if (get(colChar, rowChar) == EMPTY) {
                        moveString = "" + sourceColumnChar + sourceRowChar
                                + "-" + colChar + rowChar;
                        legalMoveList.add(moveString);
                    }
                }
            }
        }
    ...
    void clear() {
        ...
        for (int row = 0; row < EXTENDED_SIDE; row++) {
            for (int  column = 0; column < EXTENDED_SIDE; column++) {
                rowChar = (char) ('7' + 2 - row);
                colChar = (char) ('a' - 2 + column);
                if (row == 0 || row == 1 || row ==  EXTENDED_SIDE - 2
                        || row == EXTENDED_SIDE - 1
                        || column == 0 || column == 1
                        || column ==  EXTENDED_SIDE - 2
                        || column == EXTENDED_SIDE - 1) {
                    unrecordedSet(colChar, rowChar, PieceColor.BLOCKED);
                } else {
                    unrecordedSet(colChar, rowChar, PieceColor.EMPTY);
                }
            }
        }
        unrecordedSet('a', '1', PieceColor.BLUE);
        unrecordedSet('g', '7', PieceColor.BLUE);
        setPieceCount(BLUE, 2);
    ...
    PieceColor getWinner() {
        PieceColor opponentPiece = _whoseMove.opposite();
        PieceColor myPiece = _whoseMove;
        int myPiece1 = _numPieces[myPiece.ordinal()];
        int opponentPiece1 = _numPieces[opponentPiece.ordinal()];
        if (!canMove(RED) && !canMove(BLUE)) {
            if (myPiece1 == opponentPiece1) {
                _winner = EMPTY;
            }
            if (myPiece1 > opponentPiece1) {
                _winner = myPiece;
            }
            if (opponentPiece1 > myPiece1) {
                _winner = opponentPiece;
            }
        }
        if (_numJumps == JUMP_LIMIT || myPiece1 == 0 || opponentPiece1 == 0) {
            if (myPiece1 == opponentPiece1) {
                _winner = EMPTY;
            }
            if (myPiece1 > opponentPiece1) {
    ...
    private void setPieceCount(PieceColor color, int k) {
    ...
    public void unrecordedSet1(int sq, PieceColor v) {
        _board[sq] = v;
    }
     ...
     * @param playerMove that player's move and whether the game is over. */
    boolean canMove(PieceColor playerMove) {
        for (int row = 0; row < _board.length; row++) {
            if (_board[row] == playerMove) {
                for (int column = row - 2; column <= row + 2; column += 1) {
                    for (int index = column - EXTENDED_SIDE * 2;
                         index <= column + EXTENDED_SIDE * 2;
                         index += EXTENDED_SIDE) {
    ...
    void makeMove(Move move) {
        ...
        PieceColor myPiece = _whoseMove;
        if (move.isExtend()) {
            set(move.toIndex(), myPiece);
            _numJumps = 0;
            incrPieces(myPiece, 1);
        }
        if (move.isJump()) {
        ...
        int destinationSq = index(move.col1(), move.row1());
        int setSq;
        set(destinationSq, myPiece);
        int flippedPieces = 0;
        for (int dc = -1; dc < 2; dc++) {
            for (int dr = -1; dr < 2; dr++) {
                setSq = neighbor(destinationSq, dc, dr);
                if (_board[setSq] == opponentPiece) {
                    set(setSq, myPiece);
                    flippedPieces++;
                }
            }
        }
        incrPieces(myPiece, flippedPieces);
        incrPieces(opponentPiece, -flippedPieces);
        _whoseMove = opponentPiece;
        announce();
    }

    void fakeMakeMove(Move move) {
        if (!legalMove(move)) {
            throw error("Illegal move: %s", move);
        }
        if (move.isPass()) {
        ...
        PieceColor myPiece = _whoseMove;
        if (move.isExtend()) {
            unrecordedSet(move.toIndex(), myPiece);
            incrPieces(myPiece, 1);
            _numJumps = 0;
        }
        if (move.isJump()) {
        ...
        int destinationSq = index(move.col1(), move.row1());
        int setSq;
        unrecordedSet(destinationSq, myPiece);
        int flippedPieces = 0;
        for (int dc = -1; dc < 2; dc++) {
            for (int dr = -1; dr < 2; dr++) {
                setSq = neighbor(destinationSq, dc, dr);
                if (_board[setSq] == opponentPiece) {
                    unrecordedSet(setSq, myPiece);
                    flippedPieces++;
                }
            }
        }
    ...
    void pass() {
        ...
        _undoSquares.add(_allMoves.size());
    ...
    void undo() {
        ...
        PieceColor prevColor, currColor;
        tempLocation = _undoSquares.pop();
        currColor = _board[tempLocation];
        prevColor = _undoPieces.pop();
        while (tempLocation != null) {
            unrecordedSet(tempLocation, prevColor);
    ...
    void setBlock(char c, char r) {
        ...
        int row = (linearized / EXTENDED_SIDE);
        int col = (linearized % EXTENDED_SIDE);
        char rowChar = (char) ('7' + 2 - row);
        char colChar = (char) ('g' + 2 - col);
        if (get(c, rowChar) != BLOCKED) {
            unrecordedSet(c, rowChar, BLOCKED);
            _totalOpen--;
        }
        if (get(colChar, r) != BLOCKED) {
            unrecordedSet(colChar, r, BLOCKED);
            _totalOpen--;
        }
        if (get(colChar, rowChar) != BLOCKED) {
            unrecordedSet(colChar, rowChar, BLOCKED);
            _totalOpen--;
            _totalOpen--;
        }
